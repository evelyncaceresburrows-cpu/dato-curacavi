 ...
 *
 *  Las keywords se chequean en orden de especificidad: panorama/cultura
 *  van antes de dulces porque "panorama" contiene "pan" y antes hacían
 *  match falso → "dulces".
 */
function detectarCategoriaPicada(texto: string): CategoriaComercio | undefined {
  const n = normalizarTexto(texto);
  // Panoramas y cultura primero (palabras compuestas que contienen "pan").
  ...
  // Dulces: usamos "\bpan\b" (palabra completa) para no matchear "panorama".
  if (n.includes("dulce") || n.includes("empolvado") || n.includes("alfajor") || /\bpan(es)?\b/.test(n) || n.includes("kuchen") || n.includes("panaderia")) return "dulces";
